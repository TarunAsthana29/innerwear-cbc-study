...
function TaskCard({ opt, label, selected, onSelect }) {
  ...
  return (
    <div onClick={onSelect} style={{
      ...
      {selected && (
        <div style={{ position: "absolute", top: -10, right: 10, background: color, color: "#fff", fontSize: 10, fontWeight: 700, padding: "2px 8px", borderRadius: 20 }}>Selected</div>
      )}
      ...
      <div style={{ marginTop: 8, background: `${color}15`, borderRadius: 6, padding: "5px 8px", fontSize: 11, color, fontWeight: 600 }}>{opt.u}</div>
    </div>
  );
}
...
export default function Survey({ onComplete }) {
  ...
  const [choices, setChoices] = useState(new Array(12).fill(null));
  ...
  function next() {
    if (current < 11) setCurrent(c => c + 1);
    ...
    <div style={{ maxWidth: 640, margin: "0 auto", padding: "1.5rem 1rem" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between" }}>
        <button onClick={() => setCurrent(c => c - 1)} disabled={current === 0}
          style={{ padding: "10px 20px", border: "1.5px solid #e0e0e0", borderRadius: 8, background: "transparent", color: current === 0 ? "#ccc" : "#444", fontSize: 13, cursor: current === 0 ? "default" : "pointer" }}>
          ← Back
        </button>
