...
export default function Dashboard({ responses, onClearData, maxResponses }) {
  ...
  const noneRate = Math.round((responses.flatMap(r => r.choices).filter(c => c === "none").length / ((n * 9) || 1)) * 100);
  ...
  function downloadReport() {
    ...
    lines.push("Avg price chosen: ₹" + avgPrice);
    lines.push("None rate: " + noneRate + "%");
    ...
    BRANDS.forEach(b => {
      ...
      lines.push("  " + b + ": " + Math.round((cnt / totalChoices) * 100) + "% (" + cnt + ")");
    });
    lines.push("");
    lines.push("FABRIC / USP PREFERENCE (raw choice share)");
    USPS.forEach(u => {
      const cnt = allChoices.filter(c => c.u === u).length;
      lines.push("  " + u + ": " + Math.round((cnt / totalChoices) * 100) + "% (" + cnt + ")");
    });
    lines.push("");
    lines.push("NCCS BREAKDOWN");
    NCCS.forEach(nc => {
      const cnt = nccsBreakdown[nc] || 0;
      const target = { "NCCS A1": 100, "NCCS A2": 150, "NCCS A3": 150, "NCCS B1": 100 }[nc];
      lines.push("  " + nc + ": " + cnt + " (" + Math.round((cnt / Math.max(n, 1)) * 100) + "%) — target " + target);
    });
    lines.push("");
    lines.push("CITY TIER");
    ["Tier 1", "Tier 2", "Tier 3"].forEach(t => {
      const cnt = tierBreakdown[t] || 0;
      lines.push("  " + t + ": " + cnt + " (" + Math.round((cnt / Math.max(n, 1)) * 100) + "%)");
    });
    lines.push("");
    lines.push("PLATFORM");
    PLATFORMS.forEach(p => {
      const cnt = platformBreakdown[p] || 0;
    ...
    [300, 450, 600, 800].forEach(price => {
      ...
      lines.push("  ₹" + price + ": " + Math.round((cnt / totalChoices) * 100) + "% (" + cnt + ")");
    });
    lines.push("");
    lines.push("NOTE: Part-worth utilities and WTP in the Pooled MNL & WTP tab.");
    lines.push("=".repeat(50));
    ...
    a.href = url;
    a.download = "Innerwear_CBC_Report_" + now.toISOString().slice(0, 16).replace(/[T:]/g, "-") + ".txt";
    a.click();
  }
          ...
          {n === 0 ? (
            ...
            <>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: "1.5rem" }}>
                ...
                <div style={{ background: "#fff", border: "1px solid #f0f0f0", borderRadius: 10, padding: "1rem" }}>
                  <div style={{ fontSize: 12, fontWeight: 700, color: "#111", marginBottom: 12, textTransform: "uppercase", letterSpacing: 0.5 }}>Fabric / USP preference</div>
                  {USPS.map((u, i) => <Bar key={u} label={u} count={uspCounts[u]} total={totalChoices} color={["#1D9E75", "#185FA5", "#D85A30", "#534AB7"][i]} />)}
                </div>
              </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: "1.5rem" }}>
                <div style={{ background: "#fff", border: "1px solid #f0f0f0", borderRadius: 10, padding: "1rem" }}>
                  ...
                  {NCCS.map(nc => {
                    ...
                    return (
                      <div key={nc} style={{ marginBottom: 12 }}>
                        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                ...
                <div style={{ background: "#fff", border: "1px solid #f0f0f0", borderRadius: 10, padding: "1rem" }}>
                  ...
                  {["Tier 1", "Tier 2", "Tier 3"].map(t => <Bar key={t} label={t} count={tierBreakdown[t] || 0} total={n || 1} color="#0F6E56" />)}
                </div>
              </div>
              ...
              <div style={{ background: "#fff", border: "1px solid #f0f0f0", borderRadius: 10, padding: "1rem", marginBottom: "1.5rem" }}>
                ...
                {PLATFORMS.map(p => <Bar key={p} label={p} count={platformBreakdown[p] || 0} total={n || 1} color="#BA7517" />)}
              </div>
